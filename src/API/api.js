...
import {getMockedDataFromID} from "../Mock/MockUtils";
...
const getUSER = async (userId) => {
  if(BASE_URL == null) { return { data: { data: getMockedDataFromID(userId, mockedUserData[0]) } } }
  return axios.get(BASE_URL+ "/" + userId)
}
const getUserACTIVITY = async (userId) => {
  if(BASE_URL == null) { return { data: { data:getMockedDataFromID(userId,  mockedUserData[1]) } } }
  return axios.get(BASE_URL+ "/" + userId + "/activity")
}
const getUserSESSIONS = async (userId) => {
  if(BASE_URL == null ) { return { data: { data: getMockedDataFromID(userId, mockedUserData[2]) } } }
  return axios.get(BASE_URL+ "/" + userId + "/average-sessions")
}
const getUserPERFORMANCE = async (userId) => {
  if(BASE_URL == null ) { return { data: { data: getMockedDataFromID(userId, mockedUserData[3]) } } }
